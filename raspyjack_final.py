...
try:
    ...
    def get_dns_spoof_ip(interface=None):
        try:
            ...
            return None
    def set_insomniaBox_interface(interface):
    ...
    try:
        while True:
            ...
            if btn == "KEY_UP_PIN":
                ...
                row = (row + 1) % len(keypad)
            elif btn == "KEY_LEFT_PIN" or btn == "KEY1_PIN":
...
def GetMenuString(inlist, duplicates=False):
    ...
    while True:
        ...
        if btn == "KEY_DOWN_PIN" or btn == "KEY3_PIN":
            ...
            raw = inlist[index]
            if empty: return (-2, "") if duplicates else ""
            if duplicates:
                idx, txt = raw.split("#", 1)
                return int(idx), txt
            return raw
        ...
        if selection and not selection.startswith("Select") and selection.strip() and not selection.startswith(" "):
            ...
            if len(parts) >= 2:
                ...
                if selected_iface.startswith('wlan'):
                    ...
                    success = set_insomniaBox_interface(selected_iface)
...
def quick_wifi_toggle():
    ...
    try:
        ...
        success = set_insomniaBox_interface(target)
...
def GetMenuGrid(inlist, duplicates=False):
    ...
    while True:
        ...
        # Handle button input
        btn = getButton()
        if btn == "KEY_UP_PIN" or btn == "KEY1_PIN":
            if index >= GRID_COLS: index -= GRID_COLS
        elif btn == "KEY_DOWN_PIN" or btn == "KEY3_PIN":
            if index + GRID_COLS < total: index += GRID_COLS
        elif btn == "KEY_LEFT_PIN":
            if index > 0 and index % GRID_COLS != 0: index -= 1
        elif btn == "KEY_RIGHT_PIN" or btn == "KEY2_PIN":
            if index < total - 1 and (index + 1) % GRID_COLS != 0: index += 1
        elif btn == "KEY_PRESS_PIN":
            if index < total:
                m.select = index
                return inlist[index] if not duplicates else inlist[index].split("#", 1)[1]
