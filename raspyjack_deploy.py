...
def _draw_toolbar():
    try:
        ...
        if _wifi_connected:
            try:
                _tb_icon = lambda *a: ImageFont.load_default()('', S(8))
...
def _show_lock_wake_screen(reason: str = "Locked") -> None:
    try:
        ...
        lock_icon_font = lambda *a: ImageFont.load_default()('', S(28))
...
def _draw_lock_screensaver_frame(frame: Image.Image) -> None:
    try:
        ...
        image.paste(frame)
        lock_icon = MENU_ICONS.get(" Lock", "\uf023")
        lock_icon_font = lambda *a: ImageFont.load_default()('', S(14))
...
def RenderMenuCarouselOnce(inlist, selected_index=0):
    ...
    try:
        ...
        huge_icon_font = lambda *a: ImageFont.load_default()('', S(48))
        draw.text((main_x, main_y - S(12)), icon, font=huge_icon_font, fill=color.selected_text, anchor="mm")

        title = current_item.strip()
        carousel_text_font = lambda *a: ImageFont.load_default()('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf', S(12))
        draw.text((main_x, main_y + S(28)), title, font=carousel_text_font, fill=color.selected_text, anchor="mm")

        if total > 1:
            arrow_font = lambda *a: ImageFont.load_default()('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf', S(18))
...
draw = ImageDraw.Draw(image)
text_font = lambda *a: ImageFont.load_default()('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf', S(10))
icon_font = lambda *a: ImageFont.load_default()('', S(13))
